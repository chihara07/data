...
//		int			GraphFilter( int GrHandle, int FilterType = DX_GRAPH_FILTER_GRADIENT_MAP, int MapGrHandle = グラデーションマップのグラフィックハンドル( 元画像の輝度からグラデーションマップ画像の _X 座標を算出しますので縦幅は1dotでもＯＫ ), int Reverse = グラデーションマップ左右反転フラグ( TRUE : グラデーションマップを左右反転して使う  FALSE : 左右反転しない ) ) ;
...
extern	MATRIX		MGetAxis1(          VECTOR   XAxis, VECTOR   YAxis, VECTOR   ZAxis, VECTOR   Pos ) ;	// 指定の３軸ローカルのベクトルを基本軸上のベクトルに変換する行列を取得する( _X' = XAxis._X * _X + YAixs._X * _Y + ZAxis._Z * _Z + Pos._X   _Y' = XAxis._Y * _X + YAixs._Y * _Y + ZAxis._Y * _Z + Pos._Y      _Z' = XAxis._Z * _X + YAixs._Z * _Y + ZAxis._Z * _Z + Pos._Z )
extern	MATRIX_D	MGetAxis1D(         VECTOR_D XAxis, VECTOR_D YAxis, VECTOR_D ZAxis, VECTOR_D Pos ) ;	// 指定の３軸ローカルのベクトルを基本軸上のベクトルに変換する行列を取得する( _X' = XAxis._X * _X + YAixs._X * _Y + ZAxis._Z * _Z + Pos._X   _Y' = XAxis._Y * _X + YAixs._Y * _Y + ZAxis._Y * _Z + Pos._Y      _Z' = XAxis._Z * _X + YAixs._Z * _Y + ZAxis._Z * _Z + Pos._Z )
extern	MATRIX		MGetAxis2(          VECTOR   XAxis, VECTOR   YAxis, VECTOR   ZAxis, VECTOR   Pos ) ;	// 基本軸上のベクトルを指定の３軸上に投影したベクトルに変換する行列を取得する( _X' = XAxis._X * ( _X - Pos._X ) + XAxis._Y * ( _X - Pos._X ) + XAxis._Z * ( _X - Pos._X )    _Y' = YAxis._X * ( _X - Pos._X ) + YAxis._Y * ( _X - Pos._X ) + YAxis._Z * ( _X - Pos._X )    _Z' = ZAxis._X * ( _X - Pos._X ) + ZAxis._Y * ( _X - Pos._X ) + ZAxis._Z * ( _X - Pos._X ) )
extern	MATRIX_D	MGetAxis2D(         VECTOR_D XAxis, VECTOR_D YAxis, VECTOR_D ZAxis, VECTOR_D Pos ) ;	// 基本軸上のベクトルを指定の３軸上に投影したベクトルに変換する行列を取得する( _X' = XAxis._X * ( _X - Pos._X ) + XAxis._Y * ( _X - Pos._X ) + XAxis._Z * ( _X - Pos._X )    _Y' = YAxis._X * ( _X - Pos._X ) + YAxis._Y * ( _X - Pos._X ) + YAxis._Z * ( _X - Pos._X )    _Z' = ZAxis._X * ( _X - Pos._X ) + ZAxis._Y * ( _X - Pos._X ) + ZAxis._Z * ( _X - Pos._X ) )
...
extern	int			Live2D_SetUseReverseYAxis( int UseFlag ) ;																// Live2D_Model_SetTranslate で指定する平行移動値の _Y の向きを反転するかを設定する( UseFlag   TRUE:反転する( デフォルト )   FALSE:反転しない )
